defmodule Spore.Client do
  ...
  @spec new(String.t(), non_neg_integer(), String.t(), non_neg_integer(), String.t() | nil) :: {:ok, t} | {:error, term()}
    ...
    with {:ok, socket} <- Shared.connect(to, Shared.control_port(), Shared.network_timeout_ms()) do
      ...
      case Delimited.recv_timeout(d) do
        {%{"Hello" => remote_port}, d_after} ->
          ...
          {:ok, %__MODULE__{to: to, local_host: local_host, local_port: local_port, remote_port: remote_port, auth: auth, conn: d_after}}
        {%{"Error" => message}, _} -> {:error, {:server_error, message}}
        {"Challenge", _} -> {:error, :server_requires_authentication}
        {:eof, _} -> {:error, :eof}
        {{:error, reason}, _} -> {:error, reason}
        _ -> {:error, :unexpected_initial_message}
      end
    end
  catch
  ...
  defp loop(d, state) do
    case Delimited.recv(d) do
      ...
      {%{"Connection" => id}, d2} ->
        ...
        loop(d2, state)
      {%{"Error" => err}, _d2} -> Logger.error("server error: #{err}")
      {:eof, _} -> :ok
      {{:error, _}, _} -> :ok
      _ -> loop(d, state)
    end
  ...
  defp handle_connection(id, %__MODULE__{} = state) do
    with {:ok, remote_conn} <- Shared.connect(state.to, Shared.control_port(), Shared.network_timeout_ms()) do
